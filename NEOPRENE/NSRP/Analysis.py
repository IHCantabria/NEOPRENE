...
def disaggregation_rainfall(x_series, y_series):
    ...
    y_series_daily = pd.DataFrame(y_series.groupby([(y_series.index.year),
                                                    (y_series.index.month),
                                                    (y_series.index.day)]).sum().values,
                                  index=pd.period_range(start=y_series.index[0],
                                                        end=y_series.index[-1],
                                                        freq='D'),
                                  columns=['Rain'],
                                  dtype="float64")

    # y_series_daily = y_series.resample('D').agg(pd.Series.sum, min_count=1)
    dti = pd.date_range(start=x_series.index[0], end=x_series.index[-1] +
                        timedelta(hours=23), freq="H")
    ...
    for n, date in enumerate(x_series.index[1:]):
        ...
        if np.nansum(x_j) == 0:  # day with no rain
            posi_d = np.where((results.index.year == date.year) &
                              (results.index.month == date.month) &
                              (results.index.day == date.day))[0]
            ...
            # days where observed and simulated rain is equal
            poss = np.sum(y_j_AUX[y_j_AUX.columns[np.where(x_j > 0)[0]]] > 0,
                          axis=1) == len(np.where(x_j > 0)[0])

            y_j = y_j_AUX.loc[poss].copy()
            y_j_1 = y_j_1_AUX.iloc[np.where(poss == True)[0]].copy()

            suma = np.nansum(((x_j - y_j.values)**2)+((x_j_1 - y_j_1.values)**2),
                             axis=1)
            ...
            posi_d = np.where((results.index.year == date.year) &
                              (results.index.month == date.month) &
                              (results.index.day == date.day))[0]
            posi_h_s = np.where((y_series.index.year == posi_day.year) &
                                (y_series.index.month == posi_day.month) &
...
def figure_disaggregation(hourly_disaggregation, daily_disaggregation,
                          real_series, yearmin, yearmax):
    # import matplotlib as mpl
    from matplotlib.lines import Line2D
    # import matplotlib.patches as mpatches

    t1 = str(yearmin) + '-02-01'
    t2 = str(yearmax) + '-02-28'
    ...
    hourly_disaggregation_tab = pd.DataFrame(index=hourly_disaggregation_c.index,
                                             columns=['mm', 'day', 'Hour',
                                                      'Par_Impar'])

    hourly_disaggregation_tab.iloc[:, 0] = hourly_disaggregation_c.values.reshape(-1)
    ...
    legend_elements = [Line2D([0], [0], label='Obs. (daily)', color='b',
                              linestyle='--'), Line2D([], [],
                                                      label='Disagg. (daily)',
                                                      color='r', marker='^',
                                                      linestyle='None'),
                       Line2D([0], [0], label='Disagg. (hourly) day odd',
                              color='k', marker='s', linestyle='--'),
                       Line2D([0], [0], label='Disagg. (hourly) day even',
...
class Analysis(object):
    ...
    def disaggregation_fig(self, hourly_disaggregation, daily_disaggregation,
                           real_series, yearmin, yearmax):
        self.figures.append(figure_disaggregation(hourly_disaggregation,
                                                  daily_disaggregation,
                                                  real_series,
                                                  yearmin,
                                                  yearmax))
